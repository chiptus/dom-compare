...
Collector.prototype.collectFailure = function(expected, actual) {
  ...
  if (expected && !actual) {
    msg = `${toTitleCase(typeMap[expected.nodeType])} ${this._describeNode(expected)} is missed`;
    canContinue = true;
    ...
    if (expected.nodeType === actual.nodeType) {
      if (expected.nodeName === actual.nodeName) {
        ...
        msg = handleUnequalValues(expected, actual, vExpected, vActual);
        canContinue = true;
      } else {
        //expected node name is different than actual node name
        msg = differentNodeNameMsg(expected, actual);
        canContinue = false;
      }
    } else {
      msg = differentNodeTypeMsg(expected, actual);
      canContinue = false;
    }
  }
