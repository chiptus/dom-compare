(function() {
  ...
  Collector.prototype._describeNode = function(node) {
    if (
      ...
      return (
        "'" +
        (this._options.stripSpaces ? node.nodeValue.trim() : node.nodeValue) +
        "'"
      );
  ...
  Collector.prototype.collectFailure = function(expected, actual) {
    ...
    if (!msg) {
      if (expected && !actual) {
        msg =
          typeMap[expected.nodeType].charAt(0).toUpperCase() +
          typeMap[expected.nodeType].substr(1) +
          ' ' +
          this._describeNode(expected) +
          ' is missed';
        canContinue = true;
        ...
        if (expected.nodeType == actual.nodeType) {
          if (expected.nodeName == actual.nodeName) {
            ...
            if (
              ...
              expected.nodeType != type.CDATA_SECTION_NODE
            ) {
            ...
            if (vExpected == vActual)
              ...
              switch (expected.nodeType) {
                ...
                default:
                  ...
                  throw new Error(
                    'nodeValue is not equal, but nodeType is unexpected'
                  );
              }
              canContinue = true;
            }
