...
const diffTypes = require('./diff_types');
...
Collector.prototype.collectFailure = function(expected, actual) {
  ...
  if (msg) {
    this._diff.push({
      ...
      message: msg,
      type,
      node1: simpleNode(actual),
      node2: simpleNode(expected),
    });
  }
...
function collectFailureWrapper(expected, actual, { comparators, stripSpaces }) {
  ...
  if (expected.nodeType !== actual.nodeType) {
    return ans(
      differentNodeTypeMsg(expected, actual),
      diffTypes.DIFF_NODE_TYPE,
      false
    );
  }
  ...
  if (expected.nodeName !== actual.nodeName) {
    return ans(
      differentNodeNameMsg(expected, actual),
      diffTypes.DIFF_NODE_NAME,
      false
    );
  }
