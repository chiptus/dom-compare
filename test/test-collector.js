...
describe('Error collection', function() {
  it('In case root node differs - no any other checks are made', function() {
    ...
    assert.equal(
      'Expected element \'root1\' instead of \'root2\'',
      failures[0].message
    );
  });
  ...
  describe('Attributes', function() {
    it('All attributes at single node are compared', function() {
      var doc1 = parser.parseFromString(
        '<root><a attr1=\'1\' attr2=\'2\' attr3=\'3\'></a></root>'
      );
      var doc2 = parser.parseFromString(
        '<root><a attr1=\'10\' attr2=\'2\' attr4=\'4\'></a></root>'
      );
      ...
      assert.equal(
        'Attribute \'attr1\': expected value \'1\' instead of \'10\'',
        failures[0].message
      );
  ...
  describe('Nodes comparison', function() {
    it('Differences reported by types first', function() {
      ...
      assert.equal(1, failures.length);
      assert.equal(
        'Expected node of type 1 (element) instead of 3 (text node)',
        failures[0].message
      );
    });
    ...
    describe('Text nodes', function() {
      ...
      it('Different content reported', function() {
        ...
        assert.equal(1, failures.length);
        assert.equal(
          'Expected text \'Foo\' instead of \'Bar\'',
          failures[0].message
        );
      });
    });
  ...
  describe('Custom comparison routine', function() {
    ...
    it('Custom comparison routine can skip node checking. It will be processed by common routine.', function() {
      ...
      assert.equal(
        'Attribute \'m\': expected value \'2\' instead of \'3\'',
        failures[1].message
      );
    });
  });
